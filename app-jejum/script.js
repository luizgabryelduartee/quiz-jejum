...
function toggleFasting() {
    ...
    if (state.isFasting) {
        ...
        if (state.timerSeconds < 10) {
            // Caso tenha clicado por erro logo no início (menos de 10 segundos)
            // Não faz nada além de resetar o timer visualmente
        } else if (state.timerSeconds < 3600) {
            // Jejum curto (menos de 1 hora) - Mensagem de incentivo mas sem "Vitória"
            showShortFastMessage();
        } else {
            // Jejum com sucesso (mais de 1 hora)
            showCelebration();
